...
class ItemBasedCosinSimilarity(SparseBasedAlgo):
    ...
    def __init__(self, alpha=0.5, explore_percent=0.3, selector=None):
    ...
    def fit(self, ratings, **kwargs):
        ...
        self.sim_matrix_ = cosine_similarity(self.rating_matrix_.T)
    ...
    def predict_for_user(self, user, items):
        ...
        for i in range(len(items)):
            ...
            similarity_vector = self.sim_matrix_[item_pos]

            # Locations of ratings for item_pos
            (rating_locations,) = np.where(rating_matrix_items == item_pos)

            # Locations of rated items for user
            (rating_locations_user,) = np.where(rating_matrix_users == user)
            ...
            i_rated_items = rating_matrix_items[rating_locations_user]
            i_ratings = rating_matrix_data[rating_locations_user]

            # Similarity values with all users that rated item i
            i_raters_similarities = similarity_vector[i_rated_items]
